...
describe('Circle Fitter Algorithm', () => {
  describe('Stress Test for Consistent Results', () => {
    it('should consistently achieve optimal distribution over 50 iterations', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const circle1Count = result.circlesByType.find(c => c.diameter === 75)?.count || 0;
        const circle2Count = result.circlesByType.find(c => c.diameter === 60)?.count || 0;
        const circle3Count = result.circlesByType.find(c => c.diameter === 50)?.count || 0;
        const circle4Count = result.circlesByType.find(c => c.diameter === 40)?.count || 0;
        ...
        const maxCount = Math.max(circle1Count, circle2Count, circle3Count, circle4Count);
        const minCount = Math.min(circle1Count, circle2Count, circle3Count, circle4Count);
        ...
        const isOptimal = (
          ...
          (circle1Count === 3 && circle2Count === 3 && circle3Count === 3 && circle4Count === 3) ||
          (circle1Count === 3 && circle2Count === 3 && circle3Count === 4 && circle4Count === 4) ||
      ...
      console.log('Success rate:', (optimalCount / iterations * 100).toFixed(1), '%');
      ...
      optimalResults.forEach((result, index) => {
        console.log(`${index + 1}: ${result.counts['75cm']}-${result.counts['60cm']}-${result.counts['50cm']}-${result.counts['40cm']} (${result.totalCount} total)`);
      });
  ...
  describe('Balanced Distribution Test', () => {
    it('should achieve better distribution for 14 circles in 250×250 with 5cm spacing', () => {
      ...
      const expectedBalance = circle4Count <= Math.max(circle1Count, circle2Count, circle3Count) * 3;
