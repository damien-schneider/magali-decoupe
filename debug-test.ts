...
console.log(
  "By type:",
  result.circlesByType
    .map((c) => `${c.diameter}cm: ${c.count} circles`)
    .join(", ")
);
...
const areaPerTenCmCircle =
  Math.PI * tenCmRadius * tenCmRadius +
  Math.PI * tenCmRadius * tenCmRadius * 0.1; // rough spacing
